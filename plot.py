...
def plot_confusion_matrix(conf_matrix, model_name, output_dir, class_names=None):
    if conf_matrix.shape[0] > 20:  # Simplify for large matrices
    ...
    # Add class names for Deepfake
    if class_names and len(class_names) <= 10:  # Check for Deepfake
...
def main():
    # File paths for DEEPFAKE
    datasets = {
        "DEEPFAKE": [
            ("./checkpoints/deepfake_custom/", 2),
            ("./checkpoints/deepfake_resnet50/", 2),
        ],
    }
